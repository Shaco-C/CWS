...
public interface ReviewService extends IService<Reviews> {
    //展示产品通过审核的评论
    R<Page> getApprovedReviewsByProductId(Long productId, int page, int pageSize);

    // 创建评论
    R<Reviews> createReview(String token, Reviews review);

    // 更新评论
    R<Reviews> updateReview(Long reviewId, String token, Reviews reviewDetails);
